...
pub fn resize_image_to_fit(
    ...
    // 2. Create source image from byte buffer.
    let mut src_image =
        Image::from_vec_u8(img.width(), img.height(), img.to_rgba8().into_raw(), PixelType::U8x4)?;

    // 3. Multiply alpha to avoid having gray borders in the image.
    ...
    // 5. Resize to requested size.
    let mut resizer = Resizer::new();
    resizer.resize(&src_image, &mut dst_image, None)?;

    // 6. Divide alpha back to standard RGBA.
